...
# -----------------------------------------------------------------------------
# 1b. PDF Configuration
# -----------------------------------------------------------------------------
class PDFConfiguration(models.Model):
    """
    Singleton model for configuring PDF generation (branding and layout).
    This keeps PDF-specific styling separate from website styling.
    """
    # -- BRANDING --
    ...
    header_location = models.CharField(max_length=150, default="Unit 1, Bowden Lane, Derbyshire, SK23 0DQ", blank=True)
    ...
    footer_text = models.CharField(
        max_length=200,
        default="Generated by MPE Web Site",
        blank=True
    )
    show_page_numbers = models.BooleanField(default=True)

    def save(self, *args, **kwargs):
        if not self.pk and PDFConfiguration.objects.exists():
            raise ValidationError("Only one PDF Configuration instance is allowed.")
        return super().save(*args, **kwargs)
